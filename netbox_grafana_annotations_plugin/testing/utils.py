"""
Utility functions for testing NetBox Grafana Annotations Plugin.
...
def create_test_user(username: str | None = None, permissions: list[str | None] = None,
    ...
    Example:
        ...
        ...     permissions=['netbox_grafana_annotations_plugin.view_grafanaannotations']
        ... )
    """
    if username is None:
...
def post_data(data: dict[str, Any]) -> dict[str, Any]:
    ...
    Example:
        >>> from ..models import Grafanaannotations
        >>> obj = Grafanaannotations.objects.first()
...
def create_test_device(name: str = "test-device", role_name: str = "Access Switch", role_slug: str = "access-switch"):
    """
    Create a minimal but real dcim.Device, with its required related objects,
    for use in parsing/mapping/webhook tests that need a genuine NetBox
    object to build payloads/fixtures around.
    ...
    device_type, _ = DeviceType.objects.get_or_create(
        manufacturer=manufacturer, model="C9200-48P", slug="c9200-48p"
    )
    role, _ = DeviceRole.objects.get_or_create(
        name=role_name, slug=role_slug, defaults={"color": "2196f3"}
    )
    site, _ = Site.objects.get_or_create(name="Test Site", slug="test-site", defaults={"status": "active"})

    return Device.objects.create(
        device_type=device_type, role=role, site=site, name=name, status="active"
    )
...
def get_deletable_objects(instances):
    ...
    Example:
        >>> objs = Grafanaannotations.objects.filter(name__startswith='test')
        >>> deletions = get_deletable_objects(objs)
        >>> # deletions = {'Grafanaannotations': 3, 'RelatedModel': 5}
    """
...
def assert_object_changes(test_case, instance, action: str,
    ...
    Example:
        ...
        >>> obj = Grafanaannotations.objects.create(name='Test')
